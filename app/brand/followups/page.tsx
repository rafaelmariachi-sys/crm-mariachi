...
export default async function BrandFollowupsPage() {
  ...
  followups?.forEach((f: any) => {
    groupedByStatus[f.status]?.push(f)
  })
                      ...
                      <Card key={f.id} className={cn('', status === 'aberto' && urgent && 'border-red-500/30')}>
                        <CardContent className="p-4">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            {f.due_date && (
                              <span className={cn('text-xs whitespace-nowrap', status === 'aberto' && urgent ? 'text-red-400' : 'text-muted-foreground')}>
                                {label}
                              </span>
                            )}
                          </div>
