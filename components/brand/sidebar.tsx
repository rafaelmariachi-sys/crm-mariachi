...
export function BrandSidebar({ brandName }: BrandSidebarProps) {
  ...
  const NavContent = () => (
    <>
      <div className="flex items-center gap-2 px-4 py-6 border-b border-border">
        ...
        <div>
          <p className="font-bold text-sm">{brandName || 'Marca'}</p>
          <p className="text-xs text-muted-foreground">Portal da Marca</p>
        </div>
      </div>
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 bg-card border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm">{brandName || 'Marca'}</span>
        </div>
